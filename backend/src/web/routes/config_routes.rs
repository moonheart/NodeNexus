...
use crate::agent_service::{
    AgentConfig, MessageToAgent, UpdateConfigRequest, message_to_agent::Payload as AgentPayload,
};
...
async fn update_global_agent_config(
    ...
    let all_vps_models: Vec<vps::Model> = db_services::get_vps_by_user_id(&app_state.db_pool, 1)
        .await // Assuming user_id 1 for now
        .map_err(|db_err| AppError::DatabaseError(db_err.to_string()))?;
...
pub async fn push_config_to_vps(app_state: Arc<AppState>, vps_id: i32) -> Result<(), AppError> {
    ...
    if let Some(mut state) = agent_state {
        ...
        if state.sender.send(msg).await.is_ok() {
            ...
            if let Err(e) =
                db_services::update_vps_config_status(&app_state.db_pool, vps_id, "pending", None)
                    .await
            {
            ...
            warn!(
                vps_id = vps_id,
                "Agent channel closed, could not send config."
            );
            if let Err(e) = db_services::update_vps_config_status(
                &app_state.db_pool,
                vps_id,
                "failed",
                Some(err_msg),
        ...
        warn!(
            vps_id = vps_id,
            "Agent not connected, could not send config."
        );
        if let Err(e) = db_services::update_vps_config_status(
            &app_state.db_pool,
            vps_id,
...
pub async fn get_effective_vps_config(
    ...
    let mut effective_config: AgentConfig =
        serde_json::from_value(global_config_setting_model.value)
        ...
        db_services::service_monitor_service::get_tasks_for_agent(&app_state.db_pool, vps_id)
            ...
            .map_err(|e| {
                AppError::DatabaseError(format!(
                    "Failed to get monitor tasks for agent {vps_id}: {e}"
                ))
            })?;
