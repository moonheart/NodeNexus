...
pub fn create_service_monitor_router() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route("/{id}", get(get_monitor).put(update_monitor).delete(delete_monitor))
...
async fn create_monitor(
    ...
    let user_id = 1; // Hardcoded user_id
    let created_monitor = service_monitor_service::create_monitor(&app_state.db_pool, user_id, payload).await?;
    ...
    let monitor_details = service_monitor_service::get_monitor_details_by_id(&app_state.db_pool, created_monitor.id)
        .await?
        .ok_or_else(|| AppError::NotFound("Failed to fetch created monitor details".to_string()))?;

    Ok((StatusCode::CREATED, Json(serde_json::to_value(monitor_details).unwrap())))
}
...
async fn delete_monitor(
    ...
    let affected_vps_ids = service_monitor_service::get_vps_ids_for_monitor(&app_state.db_pool, id).await?;

    let delete_result = service_monitor_service::delete_monitor(&app_state.db_pool, id, user_id).await?;
