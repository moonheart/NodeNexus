...
pub async fn batch_command_ws_handler(
    ...
    ws.on_upgrade(move |socket| handle_batch_socket(socket, app_state))
}

async fn handle_batch_socket(mut socket: WebSocket, app_state: Arc<AppState>) {
    let mut rx = app_state.batch_command_updates_tx.subscribe();
    println!("[WS Batch] New client connected for batch command updates.");
        ...
        tokio::select! {
            ...
            Ok(msg) = rx.recv() => {
                ...
                if socket.send(Message::Text(Utf8Bytes::from(msg.clone()))).await.is_err() {
                    ...
                    println!("[WS Batch] Client disconnected or error sending message. Update: {}", msg);
                    break;
                } else {
                    // println!("[WS Batch] Sent update to client: {}", msg);
                }
            }
            ...
            Some(Ok(msg)) = socket.next() => {
                ...
                println!("[WS Batch] Client disconnected or channel closed.");
                break;
            }
        }
    }
    println!("[WS Batch] Connection handler for a client finished.");
}