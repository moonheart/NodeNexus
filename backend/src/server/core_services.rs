...
use crate::agent_service::{
    ...
    CommandStatus as GrpcCommandStatus, OutputType as GrpcOutputType,
};
                ...
                if let Some(ServerPayload::AgentHandshake(handshake)) = &msg_to_server.payload {
                    ...
                    if auth_successful_for_msg {
                        ...
                        let agent_state = AgentState {
                            ...
                            sender: agent_sender.take().expect("AgentSender should be available for the first handshake"),
                        };
                        ...
                        if futures_util::SinkExt::send(&mut agent_stream, MessageToAgent {
                        ...
                        let _ = futures_util::SinkExt::send(&mut agent_stream, MessageToAgent {
                            server_message_id: server_message_id_counter,
                            payload: Some(AgentPayload::ServerHandshakeAck(ack)),
                        }).await;
