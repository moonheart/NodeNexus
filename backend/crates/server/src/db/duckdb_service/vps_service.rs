...
use crate::db::orm::entity::{Entity, Order};
...
pub async fn create_vps(
    ...
    let new_vps = vps::Model {
        id: 0, // This will be ignored by insert and generated by the database
        user_id,
        name: name.to_string(),
        ip_address: None,
        os_type: None,
    ...
    new_vps.insert(&mut pool.get().await?).await
}
...
pub async fn get_vps_by_id(
    ...
    vps::Model::find_by_id(&mut pool.get().await?, vps_id).await
}
...
pub async fn get_vps_by_ids(
    ...
    let vps_iter = stmt.query_map(&params_vec[..], |row| Ok(vps::Model::from_row(row)?))?;
...
pub async fn get_vps_by_user_id(
    ...
    vps::Model::select()
        .filter(vps::Column::UserId, vps::Model::TABLE_NAME, user_id)
        .order_by(vps::Column::CreatedAt, vps::Model::TABLE_NAME, Order::Desc)
        .all(&mut pool.get().await?)
        .await
}
