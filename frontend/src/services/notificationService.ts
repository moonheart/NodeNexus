...
export const getChannelTemplates = async (): Promise<ChannelTemplate[]> => {
    const response = await apiClient.get<ChannelTemplate[]>('/api/notifications/channels/templates');
...
export const getAllChannels = async (): Promise<ChannelResponse[]> => {
    const response = await apiClient.get<ChannelResponse[]>('/api/notifications/channels');
    return response.data;
};
...
export const createChannel = async (channelData: CreateChannelRequest): Promise<ChannelResponse> => {
    const response = await apiClient.post<ChannelResponse>('/api/notifications/channels', channelData);
    return response.data;
};
...
export const updateChannel = async (id: number, channelData: UpdateChannelRequest): Promise<ChannelResponse> => {
    const response = await apiClient.put<ChannelResponse>(`/api/notifications/channels/${id}`, channelData);
    return response.data;
};
...
export const testChannel = async (id: number, message?: string): Promise<{ message: string }> => {
    const response = await apiClient.post<{ message: string }>(`/api/notifications/channels/${id}/test`, { message });
    return response.data;
};